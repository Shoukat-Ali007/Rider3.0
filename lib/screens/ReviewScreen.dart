...
import 'package:gb_Rides/service/RideService.dart';
...
class ReviewScreenState extends State<ReviewScreen> {
  ...
  Future<void> getCurrentRequest() async {
    await getCurrentRideRequest().then((value) {
      ...
      // if(rideDone==true){
      //   print("CheckNavigate0");
      //   Future.delayed(Duration(seconds: 1),() {
      //     launchScreen(context, RideDetailScreen(orderId: widget.rideRequest.id!),isNewTask: true);
      ...
      // }else
        ...
        Future.delayed(Duration(seconds: 1),() {
          launchScreen(context, RidePaymentDetailScreen(rideId: value.id,), isNewTask: true, pageRouteAnimation: PageRouteAnimation.SlideBottomTop);
        },);
      }
    }).catchError((error) {
      log(error.toString());
